...
void Networking::handle_new_client(Uint8 id) {
	if (id != clientId_)
		Game::instance()->get_sticks().send_my_info();
}

void Networking::handle_disconnet(Uint8 id) {
	Game::instance()->get_sticks().removePlayer(id);
}
...
void Networking::handle_player_state(const PlayerStateMsg &m) {
	...
	if (m._client_id != clientId_) {
		Game::instance()->get_sticks().update_player_state(m._client_id, m.x,
				m.y, m.w, m.h, m.rot);
	}
}
...
void Networking::handle_dead(const MsgWithId &m) {
	Game::instance()->get_sticks().killPlayer(m._client_id);
}
...
void Networking::handle_player_info(const PlayerInfoMsg &m) {
	if (m._client_id != clientId_) {
		Game::instance()->get_sticks().update_player_info(m._client_id, m.x,
...
void Networking::handle_restart() {
	Game::instance()->get_sticks().bringAllToLife();
