...
public:
	...
	// Returns the component that corresponds to position T, casting it
	...
	// return true if there is a component T
	//
	template<typename T>
	...
	//
	inline void addToGroup(grpId_t gId) {
	...
	//
	inline void removeFromGroup(grpId_t gId) {
		assert(gId < ecs::maxGroupId);
	...
	//
	inline bool hasGroup(grpId_t gId) {
		assert(gId < ecs::maxGroupId);
